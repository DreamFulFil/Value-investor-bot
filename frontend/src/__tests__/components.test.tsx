...
vi.mock('recharts', async (importActual) => {
    const OriginalModule = await importActual<typeof import('recharts')>();
    return {
        ...OriginalModule,
...
describe('BacktestChart', () => {
    ...
    it('renders loading state initially', () => {
        vi.spyOn(api, 'getBacktestChartData').mockReturnValue(new Promise(() => {}));
        render(<BacktestChart />, { wrapper });
        expect(screen.getByText('Loading chart...')).toBeInTheDocument();
    });

    it('renders error state on fetch failure', async () => {
        vi.spyOn(api, 'getBacktestChartData').mockRejectedValue(new Error('API Error'));
        render(<BacktestChart />, { wrapper });
    ...
    it('renders the chart with data on successful fetch', async () => {
        ...
        vi.spyOn(api, 'getBacktestChartData').mockResolvedValue(mockData);
        render(<BacktestChart />, { wrapper });
